    ...
    <div align="center" style="margin-top: 5px">
        <select name="toalloted" class="list-group-item">
         ...
         <option selected disabled>Select Teacher</option>';
                while ($row = mysqli_fetch_assoc($q)) {
                    $mystring .= '<option value="' . $row['tid'] . '">' . $row['fname'] ." ". $row['lname'] . '</option>';
                }
                ...
                function (row) {
                    return function () {
                        ...
                        if (confirm("Are You Sure?") == true) {
                            window.location.href = "deletepracallotment.php?name=" + id;
                        }
...
<table id=allotedpracticalstable>
    ...
    while ($row = mysqli_fetch_assoc($q)) {
        ...
        $t1 = mysqli_query($con,
            "SELECT * FROM teacher_login WHERE tid = '$teacher_id1'");
        $trow1 = mysqli_fetch_assoc($t1);
        $t2 = mysqli_query($con,
            "SELECT * FROM teacher_login WHERE tid = '$teacher_id2'");
        $trow2 = mysqli_fetch_assoc($t2);
       
        echo "<tr><td>{$row['subject_code']}</td>
                    <td>{$row['subject_name']}</td>
                    <td>{$row['allotedto']}</td>
                    <td>{$trow1['fname']} {$trow1['lname']}</td>
                    <td>{$row['allotedto2']}</td>
                    <td>{$trow2['fname']} {$trow2['lname']}</td>
